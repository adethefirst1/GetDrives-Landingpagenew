import { Link } from "react-router-dom";
import { trackEvent } from "../lib/analytics";
import Logo from "./Logo";

const aboutCompany = ["About Us", "Support", "Press", "Blog"];
const aboutServices = ["Get Ride", "Get Lift", "Delivery", "Renting"];
const aboutDrivers = [
  "Sign up to drive",
  "Earnings",
  "Support",
  "Safety",
];

const homeCompany = ["About Us", "Careers", "Press", "Blog"];
const homeServices = ["City Rides", "Premium", "Delivery", "Business"];
const homeDrivers = [
  "Sign Up to Drive",
  "Earnings",
  "Support",
  "Safety",
];

const SOCIAL_PLACEHOLDER_LINKS = {
  instagram:
    "https://www.instagram.com/getdrives?igsh=Ymw5YWh0ZDJrczFu",
  x: "https://x.com/getdrives",
  linkedin: "https://www.linkedin.com/company/getdrives",
};

function IconInstagram({ className }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
    </svg>
  );
}

function IconX({ className }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  );
}

function IconLinkedIn({ className }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  );
}

function FollowUs() {
  const socialLinkClass =
    "inline-flex text-brand-gray transition-colors hover:text-white focus-visible:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30 focus-visible:ring-offset-2 focus-visible:ring-offset-brand-bg rounded-sm";
  return (
    <div className="mt-8">
      <h3 className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-white">
        Follow us
      </h3>
      <div className="flex flex-wrap items-center gap-5">
        <a
          href={SOCIAL_PLACEHOLDER_LINKS.instagram}
          target="_blank"
          rel="noopener noreferrer"
          className={socialLinkClass}
          aria-label="GetDrives on Instagram"
        >
          <IconInstagram className="h-5 w-5" />
        </a>
        <a
          href={SOCIAL_PLACEHOLDER_LINKS.x}
          target="_blank"
          rel="noopener noreferrer"
          className={socialLinkClass}
          aria-label="GetDrives on X (Twitter)"
        >
          <IconX className="h-5 w-5" />
        </a>
        <a
          href={SOCIAL_PLACEHOLDER_LINKS.linkedin}
          target="_blank"
          rel="noopener noreferrer"
          className={socialLinkClass}
          aria-label="GetDrives on LinkedIn"
        >
          <IconLinkedIn className="h-5 w-5" />
        </a>
      </div>
    </div>
  );
}

function FooterLink({ href, to, children, navLabel }) {
  const className =
    "text-sm font-normal text-[#666666] transition-colors hover:text-white";
  if (to) {
    return (
      <Link
        to={to}
        className={className}
        onClick={() => {
          if (navLabel) trackEvent("nav_click", { label: navLabel });
        }}
      >
        {children}
      </Link>
    );
  }
  return (
    <a
      href={href}
      className={className}
      onClick={(e) => {
        e.preventDefault();
      }}
    >
      {children}
    </a>
  );
}

function FooterColumn({ title, children }) {
  return (
    <div className="text-left">
      <h3 className="mb-4 text-xs font-bold uppercase tracking-[0.2em] text-white">
        {title}
      </h3>
      <ul className="space-y-2.5">{children}</ul>
    </div>
  );
}

export default function Footer({ variant = "home" }) {
  const isAbout = variant === "about";

  if (isAbout) {
    return (
      <footer className="border-t border-white/10 bg-brand-bg py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid gap-10 md:grid-cols-2 md:gap-12 lg:grid-cols-4 lg:gap-12">
            <div className="max-w-sm">
              <Link
                to="/"
                className="inline-block"
                aria-label="GetDrives home"
                onClick={() => trackEvent("nav_click", { label: "HOME" })}
              >
                <Logo />
              </Link>
              <p className="mt-4 text-sm font-normal leading-relaxed text-[#666666]">
                We&apos;re here to reinvent transportation. One ride at a time.
                Join the revolution. Move with us.
              </p>
              <FollowUs />
            </div>
            <FooterColumn title="COMPANY">
              {aboutCompany.map((label) => (
                <li key={label}>
                  {label === "About Us" ? (
                    <FooterLink to="/about" navLabel={label}>
                      {label}
                    </FooterLink>
                  ) : label === "Support" ? (
                    <FooterLink to="/about#contact" navLabel={label}>
                      {label}
                    </FooterLink>
                  ) : (
                    <FooterLink href="#">{label}</FooterLink>
                  )}
                </li>
              ))}
            </FooterColumn>
            <FooterColumn title="SERVICES">
              {aboutServices.map((label) => (
                <li key={label}>
                  <FooterLink to="/services" navLabel={label}>
                    {label}
                  </FooterLink>
                </li>
              ))}
            </FooterColumn>
            <FooterColumn title="DRIVERS">
              {aboutDrivers.map((label) => (
                <li key={label}>
                  {label === "Sign up to drive" ? (
                    <FooterLink to="/drive-with-us" navLabel={label}>
                      {label}
                    </FooterLink>
                  ) : label === "Support" ? (
                    <FooterLink to="/about#contact" navLabel={label}>
                      {label}
                    </FooterLink>
                  ) : (
                    <FooterLink href="#">{label}</FooterLink>
                  )}
                </li>
              ))}
            </FooterColumn>
          </div>
          <div className="mt-16 flex flex-col gap-4 border-t border-white/10 pt-8 md:flex-row md:items-center md:justify-between">
            <p className="text-xs text-[#444444]">
              © {new Date().getFullYear()} GetDrives. All rights reserved.
            </p>
            <div className="flex flex-wrap items-center gap-x-1 text-xs text-[#444444]">
              <a href="#" className="hover:underline" onClick={(e) => e.preventDefault()}>
                Privacy Policy
              </a>
              <span aria-hidden> · </span>
              <a href="#" className="hover:underline" onClick={(e) => e.preventDefault()}>
                Terms of Service
              </a>
              <span aria-hidden> · </span>
              <a href="#" className="hover:underline" onClick={(e) => e.preventDefault()}>
                Cookie Policy
              </a>
            </div>
          </div>
        </div>
      </footer>
    );
  }

  return (
    <footer className="border-t border-white/10 bg-brand-bg py-16 md:py-24">
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid gap-10 md:grid-cols-2 md:gap-12 lg:grid-cols-4 lg:gap-12">
          <div className="max-w-sm">
            <Link
              to="/"
              className="inline-block"
              aria-label="GetDrives home"
              onClick={() => trackEvent("nav_click", { label: "HOME" })}
            >
              <Logo />
            </Link>
            <p className="mt-4 text-sm font-normal leading-relaxed text-[#666666]">
              Nigeria&apos;s peer-to-peer mobility platform. You set the fare. You
              choose the driver. Move on your terms.
            </p>
            <FollowUs />
          </div>

          <FooterColumn title="COMPANY">
            {homeCompany.map((label) => (
              <li key={label}>
                {label === "About Us" ? (
                  <FooterLink to="/about" navLabel={label}>
                    {label}
                  </FooterLink>
                ) : (
                  <FooterLink href="#">{label}</FooterLink>
                )}
              </li>
            ))}
          </FooterColumn>

          <FooterColumn title="SERVICES">
            {homeServices.map((label) => (
              <li key={label}>
                <FooterLink to="/services" navLabel={label}>
                  {label}
                </FooterLink>
              </li>
            ))}
          </FooterColumn>

          <FooterColumn title="DRIVERS">
            {homeDrivers.map((label) => (
              <li key={label}>
                {label === "Sign Up to Drive" ? (
                  <FooterLink to="/drive-with-us" navLabel={label}>
                    {label}
                  </FooterLink>
                ) : label === "Support" ? (
                  <FooterLink to="/about#contact" navLabel={label}>
                    {label}
                  </FooterLink>
                ) : (
                  <FooterLink href="#">{label}</FooterLink>
                )}
              </li>
            ))}
          </FooterColumn>
        </div>

        <div className="mt-16 flex flex-col gap-4 border-t border-white/10 pt-8 md:flex-row md:items-center md:justify-between">
          <p className="text-left text-xs text-[#444444]">
            © 2025 GetDrives. All rights reserved.
          </p>
          <div className="flex flex-wrap items-center gap-x-1 text-xs text-[#444444] md:justify-end">
            <a href="#" className="hover:underline" onClick={(e) => e.preventDefault()}>
              Privacy Policy
            </a>
            <span aria-hidden> · </span>
            <a href="#" className="hover:underline" onClick={(e) => e.preventDefault()}>
              Terms of Service
            </a>
            <span aria-hidden> · </span>
            <a href="#" className="hover:underline" onClick={(e) => e.preventDefault()}>
              Cookie Policy
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
}
